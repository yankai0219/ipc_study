...
**     Now, I finally find a way to show the signal of parent is 
*      inherited by child parent.
**     Methods: 
**       1)the following example with comments show SIGQUIT is pending.
**       2)when comments is removed, SIGQUIT is not pending.
**     some concepts:
**       1) pending signal
*/
...
int main()
{
    ...
    if ((pid = fork()) == 0) { 
        ...
        //     return -1;
        // } else {
        //     printf("child has clear the signal block\n");
        // }
   
        printf("sleep now, please insert CTRL+\\\n");
        sleep(3);
        ...
        if (sigismember(&pending_set, SIGQUIT)) {
            printf("\n CHILD SIGQUIT pending\n");
        } else {
            printf("\n CHILD SIGQUIT NOT pending\n");
        }
        ...
        sigpending(&pending_set);
        if (sigismember(&pending_set, SIGQUIT)) {
            printf("\n PARENT SIGQUIT pending\n");
        } else {
            printf("\n PARENT SIGQUIT NOT pending\n");
        }
